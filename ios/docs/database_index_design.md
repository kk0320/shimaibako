...
これにより、スクロール、サムネイル更新、タブ移動のたびに28,000件規模の配列へ `.filter { }.count` をかけ直す経路を避ける。レコード保存後は集計キャッシュを更新する。

検索結果そのものは、現段階では読み込み済み `PhotoAsset` の絞り込みを残している。SQLiteの `searchLocalIdentifiers(matching:)` は用意しているため、次段階では検索結果もDB側のIDページングへ寄せる。

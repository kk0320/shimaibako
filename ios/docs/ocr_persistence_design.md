...
初期実装では外部依存のないCodable JSONを使う。既存互換のため、OCR結果本体は引き続き `ocr_results.json` に保存する。
...
OCR結果とは別に、検索、集計、仮想フォルダ用の検索インデックスを `PhotoIndexStore` が保存する。

- 保存先: `Application Support/ShimaiBako/photo_index.json`
- 保存形式: JSON version 2
- 主キー: `localIdentifier`
- 保存内容: 撮影日、メディア種別、画像サイズ、スクショ判定、推定カテゴリ、カテゴリ信頼度、OCR状態、OCRテキスト、OCR言語、処理日時、失敗理由、最終確認日時
- Store境界: `PhotoIndexStoring`
- 現在の実装: `JSONPhotoIndexStore`

OCRが完了または失敗した写真は、`PhotoIndexService` がOCR結果、PhotoKitメタデータ、カテゴリを同じ `localIdentifier` にまとめて保存する。将来SQLiteまたはSwiftDataへ移行する時は、`PhotoIndexStoring` の実装を差し替える。
...
- OCR結果と検索インデックスは `localIdentifier` を主キーにする
- `photo_index.json` には写真本体やサムネイル本体を保存しない
- 将来のSQLiteまたはSwiftData移行を前提に、保存処理をStore層に閉じ込める

## 今後の移行方針

件数が増えた段階で、`PhotoIndexStoring` の境界を維持したままSwiftDataまたはSQLiteへ移行できる。

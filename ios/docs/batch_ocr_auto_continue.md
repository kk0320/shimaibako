...
端末状態で `pausedDeviceCondition` になった場合は、条件が回復した時に自動再開を試みる。ユーザー操作で `pausedUser` になった場合は自動再開しない。
...
- thermal state
- 空き容量
- アプリのforeground / active状態

評価タイミング:

- アプリがforeground / activeへ戻った時
- バッテリー状態が変わった時
- 低電力モードが変わった時
- thermal stateが変わった時
- 読取タブ表示時
...
条件が悪い場合は「自動再開待機中」として表示し、条件が整うと既存Jobの残り、または次の2,000件BatchOCRJobを開始する。

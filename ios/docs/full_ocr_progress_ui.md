...
進捗の分母分子は、成功件数だけでなく失敗込みの処理済み件数でそろえる。

- 成功件数: `文字あり + 文字なし`
- 処理済み件数: `文字あり + 文字なし + 失敗`
- 完了率: `処理済み件数 / total`

完走後は、速度、推定残り時間、現在phase、Heartbeat状態、停止警告、終了ボタンを出さない。完了カードは `処理済み n / total件`、文字あり、文字なし、失敗、元写真・元動画を変更していない説明だけにする。
...
Heartbeat停止表示は、ワーカーが動いているべき状態に限定する。`completed`、`paused`、`failed`、`cancelled` では古いHeartbeatを「停止」と見なさない。
...
- 写真一覧配列を進捗更新で再生成しない
- カテゴリ件数を進捗更新で再計算しない
- 親の `PhotoGridView` は `OCRProgressStore.activeSnapshot` を直接読まず、進捗カードだけが購読する
- Debugログは状態、件数、phase、heartbeatだけにする

DEBUG buildでは、写真データやVision OCRに触れないダミー進捗ジョブで、レイアウトとhit testingだけを切り分けて確認できるようにする。
...
管理メニューの有無は `OCRProgressStore.activeSnapshot` で判断する。壊れた空ジョブや有効でない完了ジョブは `activeSnapshot` にしないため、`全数OCRを管理` を出さない。有効な完走サマリがある場合だけ `全数OCRの結果` として小さく表示する。

`completed` かつ `total = 0` のジョブは完走サマリではない。`0 / 0件 完了` カード、管理ボタン、完了ボタンを表示しない。Debug buildでは `invalidEmptyCompletedJob` としてログに残す。

クイックOCRと全数OCRの状態は分ける。クイックOCRを止めるのは、実際に `preparing`、`running`、`throttled`、`finalizing`、`cancelling` の全数OCRがある場合だけにし、空の完了ジョブや古い無効ジョブでは止めない。

アプリ内のタブ移動やSwiftUI View再描画では、クイックOCRをユーザーキャンセル扱いにしない。バックグラウンド移行時のみ安全側で中断し、`.inactive` はタブ遷移や一時的な状態変化として扱う。

...
| 検索インデックス | SQLite/JSON内のメタデータ、OCRテキスト参照、分類候補、メモ、タグ | 再構築可 | 元写真・OCR原文・手動分類を巻き込まない |
...
大量ライブラリ対応では、検索インデックスの主保存先を `photo_index.sqlite` にする。旧 `photo_index.json` は移行元、互換バックアップ、障害解析用として残す。どちらもアプリサンドボックス内の検索データであり、写真アプリ内の元写真・元動画ではない。
